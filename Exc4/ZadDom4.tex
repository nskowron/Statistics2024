...
\newcounter{page}
\forloop{page}{1}{\value{page} < \pdfximage{./plots/exc3.pdf}\pdflastximagepages+1}{%
    \includepdf[pages=\value{page}]{./plots/exc3.pdf}
}

Dla porównania - funkcja gęstości rozkładu z zadania 5 listy 8 \( \mathcal{f}_X\left(x\right) = \frac{1}{\pi\sqrt{x - x^2}} \):
\newcounter{page2}
\forloop{page2}{1}{\value{page2} < \pdfximage{./plots/exc3-arcsin.pdf}\pdflastximagepages+1}{%
    \includepdf[pages=\value{page2}]{./plots/exc3-arcsin.pdf}
}
