...
int main()
{
    {
        const int d = 1;
        std::string file_base = std::string(OUTPUT_DIRECTORY) + "Plot_" + std::to_string(d) + "_";
...
void process_results(full_simulation_results& results, const std::string& file_base)
{
    ...
    plot(results.L, results.avg_L, file_base + "L.pdf");
    ...
    { // f) l(n) / (ln n / ln ln n)
        axis<double> result;
        result.x = results.avg_L.x;
        std::transform(
            results.avg_L.y.begin(),
            results.avg_L.y.end(),
            results.avg_L.x.begin(),
            std::back_inserter(result.y),
            [](auto& l, auto& n){return l / (std::log(n) / std::log(std::log(n)));}
        );

        plot_line(result, file_base + "l)-1.pdf");
    }
    ...
    { // f) l(n) / (ln ln n / ln 2)
        ...
        plot_line(result, file_base + "l)-2.pdf");
    }
}
